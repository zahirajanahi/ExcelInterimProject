...
<body>
    ...
    <div class="p-6">
        <h2 class="text-2xl font-semibold mb-6">Candidatures spontanee reçue</h2>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Niveau d'éducation</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($appli as $app)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $app->last_name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $app->first_name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $app->email }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $app->phone }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $app->address }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $app->city }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $app->dob }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $app->education_level }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $app->experience ?? 'N/A' }}</td>
...
<script>
    function updateStatus(applicationId, status) {
        if (confirm('Êtes-vous sûr de vouloir ' + (status === 'accepted' ? 'accepter' : 'rejeter') + ' cette candidature?')) {
            fetch(`/job-applications/${applicationId}/status`, {
                method: 'PATCH',
                headers: {
