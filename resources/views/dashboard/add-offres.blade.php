...
<body>
    ...
    <nav class="bg-gray-800 text-white p-4 flex justify-between items-center">
        <div class="flex items-center space-x-2">
            ...
            <a href="{{ url('/') }}" class="hover:bg-zinc-600 rounded-full px-7 py-2 ">Home</a>
            @if(auth()->user()->isSuperAdmin())
                <a href="{{ url('/dashboard') }}" class="hover:bg-zinc-600 rounded-full px-7 py-2">Users</a>
                <a href="{{ url("/add-offres-d'emploi") }}" class="hover:bg-zinc-600 rounded-full px-7 py-2">Offres d'emploi</a>
                <a href="{{ url('/postule-users') }}" class="hover:bg-zinc-600 rounded-full px-7 py-2">Postule Users</a>
                <a href="{{ url('/candidature-users') }}" class="hover:bg-zinc-600 rounded-full px-7 py-2">Candidature Users</a>
            @endif
        </div>
        <div>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="hover:bg-zinc-600 rounded-full px-7 py-2 me-7">Log Out</button>
            </form>
        </div>
    </nav>
    
    <div class="container mx-auto p-6 bg-white rounded-lg  mt-6">
        <!-- Button to Open Modal -->
        <button onclick="openModal()" class="bg-zinc-800 text-white font-bold py-2 px-4 rounded">
            Ajouter une offre d'emploi
        </button>

                      <!-- Job Offers Table -->
<div class="overflow-x-auto bg-white pt-7 ">
    <table class=" min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
            <tr >
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Publié le</th>
        ...
        <tbody class="divide-y divide-gray-200">
            @foreach ($jobOffers as $offer)
                <tr class="hover:bg-gray-100">
                    ...
                    <td class="px-6 py-4">{{ $offer->sector }}</td>
                    <td class="px-6 py-4">{{ Str::limit($offer->description, 50) }}</td>
                    <td class="px-6 py-4">{{ $offer->contract_type }}</td>
                    <td class="px-6 py-4">{{ $offer->education }}</td>
                    <td class="px-6 py-4">{{ $offer->experience }}</td>
                    <td class="px-6 py-4">{{ $offer->location }}</td>
                    <td class="px-6 py-4">{{ $offer->created_at->format('d/m/Y') }}</td>
                    <td class="px-6 py-4 flex justify-center space-x-2">
                        <a href="{{ route('add-offres.edit', $offer->id) }}" class="hover:bg-zinc-300 text-zinc-800 transition duration-500  px-3 py-2 rounded-full">
                            <i class="bi bi-pencil-square"></i>
                        </a>
                        
                        <form action="{{ route('add-offres.destroy', $offer->id) }}" method="POST" id="delete-form-{{ $offer->id }}">
                            @csrf
                            @method('DELETE')
                            <button type="submit" onclick="confirmDelete({{ $offer->id }})" class="hover:bg-red-200 font-medium text-red-800 transition duration-500  px-3 py-2 rounded-full">
    ...
    <div class="mt-4">
        ...
        <!-- Modal -->
        <div id="jobOfferModal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-gray-900 bg-opacity-50">
            <div class="bg-white p-6 rounded-lg w-1/2">
                <h2 class="text-xl font-bold mb-4">Créer une nouvelle offre d'emploi</h2>
                <form id="jobOfferForm" action="{{ route('job-offers.store') }}" method="POST">
                    @csrf
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="mb-4">
                            <label class="block font-semibold">Secteur d'activité</label>
                            <input type="text" name="sector" class="border rounded w-full p-2" required>
                        </div>
                        <div class="mb-4">
                            <label class="block font-semibold">Type de contrat</label>
                            <input type="text" name="contract_type" class="border rounded w-full p-2" required>
                        </div>
                        <div class="mb-4">
                            <label class="block font-semibold">Formation requise</label>
                            <input type="text" name="education" class="border rounded w-full p-2" required>
                        </div>
                        <div class="mb-4">
                            <label class="block font-semibold">Expérience requise</label>
                            <input type="text" name="experience" class="border rounded w-full p-2" required>
                        </div>
                        <div class="mb-4">
                    ...
                    <div class="flex justify-end">
                        <button type="button" onclick="closeModal()" class="bg-gray-500 hover:bg-gray-700 text-white px-4 py-2 rounded mr-2">Annuler</button>
                        <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white px-4 py-2 rounded">Ajouter</button>
                    </div>
                </form>
            </div>
        </div>
